...
def parse_args():
    p = argparse.ArgumentParser(description="Train marlisa on CityLearn (portable, no hardcoded paths).")
    p.add_argument(
        "--dataset-name",
    ...
    p.add_argument("--wandb-project", default=os.getenv("WANDB_PROJECT", "CityLearn-marlisa"))
...
def maybe_init_wandb(args, config):
    if args.wandb == "off" or wandb is None:
        return None
    try:
        run_name = args.wandb_run_name or f"marlisa_lr={args.lr}_beta={args.beta}"
        run = wandb.init(
...
def main():
    ...
    # Path richiesto ESATTO: outputs/save_models/marlisa/beta/lr/marlisa.zip
    save_dir = output_root / args.dataset_name / "save_models" / "marlisa" / "beta" / "lr"
    save_dir.mkdir(parents=True, exist_ok=True)
    zip_file = save_dir / "marlisa.zip"
    ...
    # --- Save: SOLO ZIP → outputs/save_models/marlisa/beta/lr/marlisa.zip ---
