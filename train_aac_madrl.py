...
def parse_args():
    p = argparse.ArgumentParser(description="Train AAC-MADRL on CityLearn (portable, no hardcoded paths).")
    p.add_argument(
        "--dataset-name",
    ...
    p.add_argument("--heating-storage", default=21, type=int, help="Numero di classi per azione heating storage.")
    ...
    p.add_argument("--wandb-project", default=os.getenv("WANDB_PROJECT", "CityLearn-AAC-MADRL"))
...
def main():
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # Passa le classi al costruttore dell'agente
    model = RLAgent(env, lr=args.lr, classes=classes)
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

    # --- Train ---
    model.learn(episodes=args.episodes)

    # --- Save: SOLO ZIP → outputs/save_models/AAC-MADRL/beta/lr/AAC-MADRL.zip ---
