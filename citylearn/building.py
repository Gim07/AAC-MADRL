...
from citylearn.energy_model import Battery, ElectricDevice, ElectricHeater, HeatPump, PV, StorageTank, GasBoiler
...
class Building(Environment):
    ...
    heating_fuel_device: GasBoiler, optional
        Fuel Device for meeting space heating demand and charging `heating_storage`.
    pv : PV, optional
    ...
    **kwargs : Any
    ...
    def __init__(
            ...
            heating_device: Union[HeatPump, ElectricHeater] = None,
            heating_fuel_device: GasBoiler = None,
            pv: PV = None, name: str = None,
        ...
        self.heating_fuel_device = heating_fuel_device
    ...
    @property
    def heating_fuel_device(self) -> GasBoiler:
        """Fuel Device for meeting space heating demand and charging `heating_storage`."""

        return self.__heating_fuel_device
    ...
    def net_electricity_consumption_emission_without_storage_and_pv(self) -> np.ndarray:
        ...
        return (self.carbon_intensity.carbon_intensity[
                    0:self.time_step + 1] * self.net_electricity_consumption_without_storage_and_pv).clip(min=0)
    ...
    def heating_fuel_consumption(self) -> np.ndarray:
        ...
        return self.heating_fuel_device.fuel_consumption[:self.time_step + 1]
    ...
    @property
    def heating_storage_electricity_consumption(self):
    ...
    @property
    def heating_storage_fuel_consumption(self):
        """`heating_storage` net fuel consumption time series, in [kWh].

        Positive values indicate `heating_fuel_device` fuel consumption to charge `heating_storage` while negative values indicate avoided `heating_fuel_device`
        fuel consumption by discharging `heating_storage` to meet `heating_demand`.
        """

        if isinstance(self.heating_fuel_device, GasBoiler):
            return self.heating_fuel_device.get_input_power(self.heating_storage.energy_balance[:self.time_step + 1])
        else:
            return np.zeros(self.time_step + 1, dtype='float32')


    @property
    def dhw_storage_electricity_consumption(self):
    ...
    @property
    def dhw_storage_fuel_consumption(self) -> np.ndarray:
        """`dhw_storage` net fuel consumption time series, in [kWh].

        Positive values indicate `dhw_device` fuel consumption to charge `dhw_storage` while negative values indicate avoided `dhw_device`
        fuel consumption by discharging `dhw_storage` to meet `dhw_demand`.
        """

        if isinstance(self.dhw_device, GasBoiler):
            return self.dhw_device.get_input_power(self.dhw_storage.energy_balance[:self.time_step + 1])
        else:
            return np.zeros(self.time_step + 1, dtype='float32')
    ...
    @property
    def energy_from_dhw_fuel_device_to_dhw_storage(self) -> np.ndarray:
        """Energy supply from `dhw_fuel_device` to `dhw_storage` time series, in [kWh]."""
    ...
    def energy_from_heating_fuel_device(self) -> np.ndarray:
        """Energy supply from `heating_fuel_device` to building time series, in [kWh]."""

        return self.__energy_from_heating_fuel_device[:self.time_step + 1]
    ...
    @property
    def energy_from_dhw_fuel_device(self) -> np.ndarray:
        """Energy supply from `dhw_fuel_device` to building time series, in [kWh]."""

        return self.__energy_from_dhw_fuel_device[:self.time_step + 1]
    ...
    def downward_electrical_flexibility(self) -> float:
        ...
        capacity = abs(self.solar_generation[self.time_step]) - (
            ...
            + self.heating_device.electricity_consumption[self.time_step] if isinstance(self.heating_device,
                                                                                        HeatPump or ElectricHeater) else np.zeros(
                self.time_step + 1, dtype='float32')
                                                                                                                         +
                                                                                                                         self.dhw_device.electricity_consumption[
                                                                                                                             self.time_step] if isinstance(
                self.dhw_device, HeatPump or ElectricHeater) else np.zeros(self.time_step + 1, dtype='float32')
                                                                  +
                                                                  self.non_shiftable_load_device.electricity_consumption[
                                                                      self.time_step]
                                                                  + self.electrical_storage.electricity_consumption[
                                                                      self.time_step]
        )
    ...
    @heating_fuel_device.setter
    def heating_fuel_device(self, heating_fuel_device: GasBoiler):
        self.__heating_fuel_device = GasBoiler(0.0) if heating_fuel_device is None else heating_fuel_device
    ...
    def demand_observation_limit_factor(self, demand_observation_limit_factor: float):
        self.__demand_observation_limit_factor = 3.0 if demand_observation_limit_factor is None else demand_observation_limit_factor
    ...
    def random_seed(self, seed: int):
        ...
        self.heating_fuel_device.random_seed = self.random_seed
    ...
    def episode_tracker(self, episode_tracker: EpisodeTracker):
        ...
        self.heating_fuel_device.episode_tracker = self.episode_tracker
    ...
    def get_metadata(self) -> Mapping[str, Any]:
        ...
        return {
            **super().get_metadata(),
            ...
            'heating_fuel_device': self.heating_fuel_device.get_metadata(),
        ...
        if self.electric_vehicle_chargers is not None:
            for charger in self.electric_vehicle_chargers:  # If present, itrerate to each charger
                ...
                if charger.connected_electric_vehicle:
                    ...
                    obs = charger.connected_electric_vehicle.observations(include_all, normalize,
    ...
    def _get_observations_data(self) -> Mapping[str, Union[float, int]]:
        return {
            ...
            **{
            ...
            'cooling_demand': self.__energy_from_cooling_device[self.time_step] + abs(
                min(self.cooling_storage.energy_balance[self.time_step], 0.0)),
            'heating_demand': self.__energy_from_heating_device[self.time_step] +
                              self.__energy_from_heating_fuel_device[self.time_step] +
                              abs(min(self.heating_storage.energy_balance[self.time_step], 0.0)),
            'dhw_demand': self.__energy_from_dhw_device[self.time_step] + abs(
                min(self.dhw_storage.energy_balance[self.time_step], 0.0)),
            ...
            'heating_electricity_consumption': self.heating_electricity_consumption[self.time_step] if isinstance(
                self.heating_device, (HeatPump, ElectricHeater)) else 0.0,
            'dhw_electricity_consumption': self.dhw_electricity_consumption[self.time_step] if isinstance(
                self.dhw_device, (HeatPump, ElectricHeater)) else 0.0,
            'heating_fuel_consumption': self.heating_fuel_consumption[self.time_step],
            'dhw_fuel_consumption': self.dhw_fuel_consumption[self.time_step] if isinstance(self.dhw_device,
                                                                                            GasBoiler) else 0.0,
            ...
            'electrical_storage_electricity_consumption': self.electrical_storage_electricity_consumption[
                self.time_step],
            'cooling_device_efficiency': self.cooling_device.get_cop(
                self.weather.outdoor_dry_bulb_temperature[self.time_step], heating=False),
            ...
            'heating_fuel_device_efficiency': self.heating_fuel_device.efficiency,
            'dhw_device_efficiency': self.dhw_device.get_cop(self.weather.outdoor_dry_bulb_temperature[self.time_step],
                                                             heating=True) \
                if isinstance(self.dhw_device, HeatPump) else self.dhw_device.efficiency,
            'indoor_dry_bulb_temperature_cooling_set_point':
                self.energy_simulation.indoor_dry_bulb_temperature_cooling_set_point[self.time_step],
            'indoor_dry_bulb_temperature_heating_set_point':
                self.energy_simulation.indoor_dry_bulb_temperature_heating_set_point[self.time_step],
            'indoor_dry_bulb_temperature_cooling_delta': self.energy_simulation.indoor_dry_bulb_temperature[
                                                             self.time_step] -
                                                         self.energy_simulation.indoor_dry_bulb_temperature_cooling_set_point[
                                                             self.time_step],
            'indoor_dry_bulb_temperature_heating_delta': self.energy_simulation.indoor_dry_bulb_temperature[
                                                             self.time_step] -
                                                         self.energy_simulation.indoor_dry_bulb_temperature_heating_set_point[
                                                             self.time_step],
    ...
    def apply_actions(self,
                      ...
                      heating_fuel_device_action: float = None,
        ...
        heating_fuel_device_action : float, default: np.nan
            Fraction of `heating_fuel_device` `nominal_power` to make available for space heating.
        ...
        **kwargs
        ...
        if 'cooling_or_heating_device' in self.active_actions:
            ...
            heating_fuel_device_action = abs(min(heating_fuel_device_action, 0.0))

        else:
            assert not ('cooling_device' in self.active_actions and ('heating_device' in self.active_actions or 'heating_fuel_device' in self.active_actions)), \
            ...
            heating_fuel_device_action = np.nan if 'heating_fuel_device' not in self.active_actions else heating_fuel_device_action
        ...
        actions = {
            ...
            'heating_demand': (self.update_heating_demand, (heating_device_action, heating_fuel_device_action)),
            ...
            'heating_fuel_device': (self.update_energy_from_heating_fuel_device, ()),
        ...
        electric_vehicle_priority_list = [f'electric_vehicle_storage_{charger.charger_id}' for charger in
    ...
    def update_energy_from_heating_device(self):
        r"""Update heating device electricity consumption and energy transfer for current time step's heating demand."""
        ...
        # print('Update heating device energy:')
        # print(
        #     'timestep:', self.time_step, 'bldg:', self.name, 'demand:', demand,
        #     'max_electric_power:', max_electric_power, 'max_device_output:', max_device_output,
        #     'max_fuel_device_output:', max_fuel_device_output
        # )
        ...
        fuel_consumption = self.heating_fuel_device.get_input_power(fuel_device_output)
        self.heating_fuel_device.update_fuel_consumption(max(0.0, fuel_consumption))

        # print('Update heating device energy:')
        # print(
        #     'timestep:', self.time_step, 'bldg:', self.name, 'demand:', demand,
        #     'storage_output:', storage_output, 'electric_output:', device_output,
        #     'fuel_output:', fuel_device_output, 'total_output:', storage_output + device_output + fuel_device_output
        # )


    def update_energy_from_heating_fuel_device(self):
        r"""Update heating fuel device fuel consumption and energy tranfer for current time step's heating demand."""
    ...
    def update_heating_storage(self, action: float):
        ...
        if energy > 0.0:
            # Prioritize heating_device (electric heat pump)
            max_electric_power = self.downward_electrical_flexibility
            max_device_output = self.heating_device.get_max_output_power(temperature, heating=True,
                                                                         max_electric_power=max_electric_power) \
                if isinstance(self.heating_device, HeatPump) else self.heating_device.get_max_output_power(
            ...
            electricity_consumption = self.heating_device.get_input_power(energy_from_electric, temperature, heating=True) \
                if isinstance(self.heating_device, HeatPump) else self.heating_device.get_input_power(energy_from_electric)
            self.heating_device.update_electricity_consumption(electricity_consumption)

            fuel_consumption = self.heating_fuel_device.get_input_power(energy_from_fuel)
            self.heating_fuel_device.update_fuel_consumption(fuel_consumption)

        else:
            # Discharge logic remains the same
            demand = self.heating_demand[self.time_step]
            energy = max(-demand, energy)
    ...
    def update_energy_from_dhw_device(self):
        r"""Update dhw device electricity consumption and energy transfer for current time step's dhw demand."""
    ...
    def estimate_observation_space_limits(self, include_all: bool = None, periodic_normalization: bool = None) -> Tuple[
        ...
        for key in observation_names:
            if key == 'net_electricity_consumption':
            ...
            elif key in ['heating_fuel_device_efficiency']:
                if isinstance(self.heating_fuel_device, GasBoiler):
                    low_limit[key] = self.heating_fuel_device.efficiency
                    high_limit[key] = self.heating_fuel_device.efficiency
                else:
                    low_limit[key] = 0.0
                    high_limit[key] = 0.0
                ...
                low_limit[key] = 0.0
                if isinstance(self.heating_device, HeatPump):
                    high_limit[key] = self.heating_device.nominal_power * 2
                else:
                    high_limit[key] = 0.0

            elif key == 'dhw_electricity_consumption':
                low_limit[key] = 0.0
                high_limit[key] = self.dhw_device.nominal_power if isinstance(self.dhw_device, (HeatPump, ElectricHeater)) else 0.0
                ...
                electricity_consumption = self.heating_device.get_input_power(demand, data['outdoor_dry_bulb_temperature'],
                                                                              True) \
            ...
            # AGGIUNGI QUESTI NUOVI BLOCCHI
                ...
                low_limit[key] = 0.0
                high_limit_heating = (self.heating_fuel_device.nominal_power if isinstance(self.heating_fuel_device,
                                                                                           GasBoiler) else 0.0)
                ...
                low_limit[key] = 0.0
                high_limit[key] = self.heating_fuel_device.nominal_power
    ...
    def estimate_action_space(self) -> spaces.Box:
        ...
        for key in self.active_actions:
            if key == 'cooling_or_heating_device':
            ...
            elif key in ['cooling_device', 'heating_device', 'heating_fuel_device']:
                low_limit.append(0.0)
                high_limit.append(1.0)
                ...
                if key == 'electrical_storage':
                    ...
                    if key == 'cooling_storage':
                        ...
                        power = self.heating_device.nominal_power + self.heating_fuel_device.nominal_power
    ...
    def autosize_heating_fuel_device(self, **kwargs):
        """Autosize `heating_fuel_device` `nominal_power` to minimum power needed to always meet `heating_demand`.

        Other Parameters
        ----------------
        **kwargs : dict
            Other keyword arguments parsed to `heating_fuel_device` `autosize` function.
        """
        ...
        self.heating_fuel_device.nominal_power = self.heating_fuel_device.autosize(demand, **kwargs)
    ...
    def next_time_step(self):
        ...
        self.heating_fuel_device.next_time_step()
    ...
    def reset(self):
        ...
        self.heating_fuel_device.reset()
    ...
    def update_variables(self):
        ...
        if self.time_step == 0:
            ...
            if isinstance(self.heating_device, HeatPump):
                ...
                heating_electricity_consumption = self.dhw_device.get_input_power(heating_device_demand)


            self.heating_device.update_electricity_consumption(heating_electricity_consumption)

            # heating fuel consumption
            heating_fuel_consumption = self.heating_fuel_device.get_input_power(heating_fuel_device_demand)
            self.heating_fuel_device.update_fuel_consumption(heating_fuel_consumption)
            ...
            if isinstance(self.dhw_device, GasBoiler):
                dhw_fuel_consumption = self.dhw_device.get_input_power(dhw_demand)

                self.dhw_device.update_fuel_consumption(dhw_fuel_consumption)
            else:
                dhw_fuel_consumption = 0.0
        ...
        net_fuel_consumption = self.heating_fuel_device.fuel_consumption[self.time_step]
        ...
        try:
            ...
            fuel_price = 0.5  # Valore di default se non implementato
        ...
        self.__net_fuel_consumption_emission[self.time_step] = max(0.0,
                                                                   net_fuel_consumption * fuel_carbon * self.seconds_per_time_step / 3600)
        # FINE AGGIUNTA
...
class DynamicsBuilding(Building):
    ...
    **kwargs : Any
    ...
    def net_electricity_consumption_without_storage_and_partial_load(self):
        ...
        # heating electricity consumption
        heating_demand_difference = self.heating_demand_without_partial_load - np.clip(self.heating_demand, None,
                                                                                       self.heating_device.nominal_power)
    ...
    def reset_dynamic_variables(self):
        ...
        self.energy_simulation.indoor_dry_bulb_temperature[
            start_ix:end_ix] = self.energy_simulation.indoor_dry_bulb_temperature_without_control.copy()[
            start_ix:end_ix]
...
class LSTMDynamicsBuilding(DynamicsBuilding):
    ...
    **kwargs : Any
    ...
    def update_indoor_dry_bulb_temperature(self):
        ...
        low_limit, high_limit = self.dynamics.input_normalization_minimum[ix], \
            self.dynamics.input_normalization_maximum[ix]
    ...
    def update_cooling_demand(self, action: float):
        ...
        if (
                'cooling_device' in self.active_actions or 'cooling_or_heating_device' in self.active_actions) and self.simulate_dynamics:
    ...
    def update_heating_demand(self, action: float, fuel_action: float = 0.0):
        """Update space heating demand for current time step.
        ...
        if ('heating_device' in self.active_actions or
            'cooling_or_heating_device' in self.active_actions or
            'heating_fuel_device' in self.active_actions) and self.simulate_dynamics:
            if self.energy_simulation.hvac_mode[self.time_step] in [2, 3]:
                available_power = self.downward_electrical_flexibility
                electric_power = action * min(self.heating_device.nominal_power, available_power)
                fuel_power = fuel_action * self.heating_fuel_device.nominal_power
                ...
                demand += self.heating_fuel_device.get_max_output_power(max_fuel_power=fuel_power)
            ...
            # print(f"Time step {self.time_step}: Heating demand set to {demand} kWh",
            #       f"Fuel Power: {self.heating_fuel_device.get_max_output_power(max_fuel_power=fuel_power)} kW,"
            #       f" Electric Power: {self.heating_device.get_max_output_power(self.weather.outdoor_dry_bulb_temperature[self.time_step],heating=True, max_electric_power=electric_power)} kW")
...
class OccupantInteractionBuilding(DynamicsBuilding):
    ...
    **kwargs : Any
    ...
    def reset_dynamic_variables(self):
        ...
        end_ix = self.episode_tracker.episode_time_steps
        self.energy_simulation.indoor_dry_bulb_temperature_cooling_set_point[
            start_ix:end_ix] = self.energy_simulation.indoor_dry_bulb_temperature_cooling_set_point_without_control.copy()[
            start_ix:end_ix]
        self.energy_simulation.indoor_dry_bulb_temperature_heating_set_point[
            start_ix:end_ix] = self.energy_simulation.indoor_dry_bulb_temperature_heating_set_point_without_control.copy()[
            start_ix:end_ix]
...
class LogisticRegressionOccupantInteractionBuilding(OccupantInteractionBuilding, LSTMDynamicsBuilding):
    def __init__(self, *args, occupant: LogisticRegressionOccupant = None, set_point_hold_time_steps: int = None,
                 **kwargs):
    ...
    def update_set_points(self):
        ...
        previous_cooling_set_point = self.energy_simulation.indoor_dry_bulb_temperature_cooling_set_point[
            self.time_step - 1]
        current_heating_set_point = self.energy_simulation.indoor_dry_bulb_temperature_heating_set_point[self.time_step]
        previous_heating_set_point = self.energy_simulation.indoor_dry_bulb_temperature_heating_set_point[
            self.time_step - 1]
        ...
        self.occupant.parameters.occupant_interaction_indoor_dry_bulb_temperature_set_point_delta[
            self.time_step] = setpoint_delta

        if abs(setpoint_delta) > 0.0 and not self.ignore_occupant:
            if hvac_mode == 1:
                self.energy_simulation.indoor_dry_bulb_temperature_cooling_set_point[
                    self.time_step:] = current_set_point + setpoint_delta
        ...
        if self.__set_point_hold_time_step_counter is not None and self.__set_point_hold_time_step_counter == 0:
            self.energy_simulation.indoor_dry_bulb_temperature_cooling_set_point[self.time_step + 1:] \
                = self.energy_simulation.indoor_dry_bulb_temperature_cooling_set_point_without_control[
                self.time_step + 1:]
            self.energy_simulation.indoor_dry_bulb_temperature_heating_set_point[self.time_step + 1:] \
                = self.energy_simulation.indoor_dry_bulb_temperature_heating_set_point_without_control[
                self.time_step + 1:]
            self.__set_point_hold_time_step_counter = None
    ...
    def reset_dynamic_variables(self):
        ...
        start_ix = 0
        end_ix = self.episode_tracker.episode_time_steps
        delta_summary = np.unique(
            self.occupant.parameters.occupant_interaction_indoor_dry_bulb_temperature_set_point_delta[start_ix:end_ix],
            return_counts=True)
        self.occupant_interaction_indoor_dry_bulb_temperature_set_point_delta_summary.append(
            [delta_summary[0].tolist(), delta_summary[1].tolist()])
        self.occupant.parameters.occupant_interaction_indoor_dry_bulb_temperature_set_point_delta[start_ix:end_ix] = \
            self.occupant.parameters.occupant_interaction_indoor_dry_bulb_temperature_set_point_delta_without_control.copy()[
                start_ix:end_ix]
